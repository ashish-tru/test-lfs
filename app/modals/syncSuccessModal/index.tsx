...
interface DispatchProps {
  showSyncSuccesModal: (payload: ShowSyncSuccesType) => void;
  showSearchAndReplaceModal: (payload: SearchReplace) => void;
}

type Props = StateProps & RouteComponentProps & DispatchProps;

class SyncSuccessModal extends React.PureComponent<Props, State> {
    ...
    return (
      <>
        <Modal
          ...
          // }}
          parentClass={classNames(Style.sync_success_main_modal)}
          customClass={classNames(Style.sync_success_modal)}
          size={Modal.Size.SMALL}
          buttongGroupClass={classNames(Style.sync_success_modal_btn)}
        >
          <div className={classNames(Style.sync_success_modal_header)}>
            <IconBox
              tooltip={false}
              customClass={classNames(Style.sync_success_modal_icon)}
              icon={getIcon('UPDATE_SUCCESS_MODAL', theme.theme_mode)}
            />
            <h1 className={Style.heading}>
              Syncing files done <br /> successfully
            </h1>
          </div>
        </Modal>
      </>
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToAction)(SyncSuccessModal)
);
