...
class SettingUpSite extends React.PureComponent<Props, State> {
  ...
  constructor(props: Props) {
    ...
    this.state = {
      currentState: [0, 100],
      project: state,
      gitUserName: localStorage.getItem('currentUser') || '',
  ...
  async init() {
    ...
    try {
      ...
      log.info('Return from Service', {
        ...args,
        credential: { ...args.credential, password: undefined },
      });
      let projectFlow = '';
      let gitFetched = true;
      debugger;
      if (state.gitUrl !== '') {
        ...
        if (args.status === SiteState.RUNNING) {
          try {
            const pathToRepository = args.response.location.code;
            await this.dispatcher.cloneIntoNonEmptyFile(
              state.gitUrl,
              pathToRepository,
              state.projectBranchName,
              gitUserName,
              (progressData: { value: number }) => {
                const { progress } = this.state;
                const arr = progress;
            ...
            if (gitFetched) {
              const id = args.response.container_name;
              const { name } = args.response;
              args.response.git_clone_url = state.gitUrl;
              args.response.git_login = state.gitLogin;
              args.response.git_branch = state.projectBranchName;
              await request(EndPoint.GIT, RegisterPackages.skip, [
                { id, name },
              ]);
            } else {
      ...
      if (args.status === SiteState.RUNNING) {
        ...
        if (state.gitUrl !== '') {
          this.props.currentProject({ ...project, showImportDbModal: true });
          this.props.addNewProject({ ...project });
        } else {
          this.props.currentProject({ ...project, showImportDbModal: false });
          this.props.addNewProject({ ...project });
